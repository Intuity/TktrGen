...
module.exports = function() {
    ...
    var state = {
        ...
        configuration: {
            ...
            code_format:  'azteccode',
            // If generating multiple tickets in parallel, need to provide offset so that file
    ...
    // Function: set_code_format
    // Set the type of code to render, either 'Aztec' or 'QR'
    this.set_code_format = function(format) {
        if      (format.toLowerCase() == 'aztec') state.configuration.code_format = 'azteccode';
        else if (format.toLowerCase() == 'qr'   ) state.configuration.code_format = 'qrcode';
        else throw Error('Invalid code format specified: ' + format);
    };

    // Function: set_index_offset
    // Set offset for generating file names (allows multiple instances to run in parallel)
    this.set_index_offset = function(index) {
        index = parseInt(index);
        if (index < 0) index = 0;
        state.configuration.index_offset = index;
    };
    ...
    this.render_template = function(template, done) {
        ...
        async.whilst(
            ...
            function(i_complete) {
                ...
                var tkt_i  = (i + state.configuration.index_offset);
                build_ticket_sheet(state.configuration.working_dir, template, ticket, tkt_i, i_complete);
            },
            function(err) {
    ...
    var build_ticket_sheet = function(folder_path, template, ticket, ticket_index, callback) {
        async.waterfall([
            ...
            function(done) {
                ...
                build_matrix_codes([code_text], done);
            },
    ...
    //
    var build_matrix_codes = function(code_texts, callback) {
        var i = -1;
        var codes = [];
        async.whilst(
            ...
            function(i_complete) {
                bwip.toBuffer({
                    bcid:           state.configuration.code_format,
                    text:           code_texts[i],
                    scale:          10,
                    includeText:    false
